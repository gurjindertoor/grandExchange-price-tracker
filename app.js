...
app.get("/items", (req, res) => {
  ...
  const sql_lookup = `
    SELECT item_name, item_id, high_price, low_price
    FROM grandExchange
    GROUP BY item_name
  ...
  db.all(sql_lookup, [], (err, items) => {
    ...
    db.get(countQuery, [], (err, row) => {
      ...
      const numOfResults = row.count;
      const numOfPages = Math.ceil(numOfResults / resultsPerPage);

      if (page > numOfPages) {
        return res.redirect(
          "/items?page=" + encodeURIComponent(numOfPages.toString())
        );
      } else if (page < 1) {
        return res.redirect("/items?page=" + encodeURIComponent("1"));
      }
...
app.get("/item/:id", (req, res) => {
  ...
  let sqlGrabInfo = `SELECT timestamp, item_id, item_name, high_price, low_price FROM grandExchange WHERE item_ID = ${itemNumber} ORDER BY timestamp ASC`;

  let sqlPriceChange = `SELECT timestamp, item_id, item_name, high_price, low_price FROM grandExchange WHERE item_ID = ${itemNumber}`;
